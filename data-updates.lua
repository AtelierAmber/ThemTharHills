...
if mods["bzchlorine"] then
  ...
  rm.ReplaceIngredient("pcb", "copper-plate", parts.gold, gold_mult, gold_mult)
...
if data.raw.item["pcb-solder"] then
  ...
  rm.ReplaceIngredient("transceiver", "solder", "pcb-solder", 1, 2)
end
...
if data.raw.fluid["hydrogen-chloride"] and not parts.aquaregia then
  if (mods["Krastorio2"] and mods["bztungsten"] and mods["BrimStuff"] and mods["FreightForwarding"]) then
    ...
    rm.ReplaceIngredient("gold-powder", "nitric-acid", "hydrogen-chloride", mods["Krastorio2"] and 10 or 2, mods["Krastorio2"] and 10 or 2)
    rm.ReplaceIngredient("trace-gold-from-copper", "nitric-acid", "hydrogen-chloride", mods["Krastorio2"] and 15 or 6, mods["Krastorio2"] and 15 or 6)
  end
end
...
if (mods["Krastorio2"] and mods["bztungsten"] and mods["FreightForwarding"]) and not (mods["BrimStuff"] or mods["bzgas"]) then
  rm.RemoveIngredient("chemical-plant", "tungsten-plate", 99999, 99999)
  rm.RemoveIngredient("chemical-plant", "copper-tungsten", 99999, 99999)
  rm.RemoveIngredient("chemical-plant", "tungsten-carbide", 99999, 99999)
end

if rm.CheckIngredient("processing-unit", "silicon") then
  rm.ReplaceIngredient("processing-unit", "silicon", "integrated-circuit", 10, 10)
else if rm.CheckIngredient("processing-unit", "silicon-wafer") then
  rm.ReplaceIngredient("processing-unit", "silicon-wafer", "integrated-circuit", 10, 10)
else
  rm.ReplaceIngredient("processing-unit", "electronic-circuit", "integrated-circuit", 10, 10)
end end
...
if rm.CheckIngredient("processing-unit", "mainboard") and not rm.CheckIngredient("mainboard", "temperature-sensor") then -- platinum is off or it's been moved to fans
  rm.RemoveIngredient("processing-unit", "integrated-circuit", 5, 5)
  rm.AddIngredient("mainboard", "integrated-circuit", 50, 50) -- batch of 10
end

if not data.raw.item["silicon-wafer"] then
  rm.AddIngredient("integrated-circuit", "sulfuric-acid", 1, 1)
  rm.SetCategory("integrated-circuit", "crafting-with-fluid")
  rm.AddIngredient("integrated-circuit-silver", "sulfuric-acid", 1, 1)
  rm.SetCategory("integrated-circuit-silver", "crafting-with-fluid")
else
  if data.raw.fluid["epoxy"] then
    rm.ReplaceIngredient("integrated-circuit", "plastic-bar", "epoxy", 2, 2)
    rm.SetCategory("integrated-circuit", "crafting-with-fluid")
  ...
  rm.RemoveIngredient("integrated-circuit", "silicon-wafer", 1, 1)
end
...
rm.AddIngredient("speed-module", parts.wire, 5, 5)
rm.AddIngredient("effectivity-module", parts.wire, 5, 5)
rm.AddIngredient("productivity-module", parts.wire, 5, 5)
...
rm.RemoveIngredient("beacon", "cooling-fan", 5, 5)

--This is intentionally done this way because I want the items added even if cables were replaced with fiber optics
rm.RemoveIngredient("beacon", "copper-cable", 10, 10)
if data.raw.recipe["beacon2-recipe"] or data.raw.recipe["beacon3-recipe"] then
  rm.AddIngredient("beacon", parts.wire, 10, 10)
  rm.RemoveIngredient("beacon2-recipe", "copper-cable", 20, 20)
  rm.RemoveIngredient("beacon3-recipe", "copper-cable", 30, 30)
  rm.AddIngredient("beacon2-recipe", "advanced-cable", 10, 10)
  rm.AddIngredient("beacon3-recipe", "advanced-cable", 15, 15)
else
...
if data.raw.item["flywheel"] then
  rm.RemoveIngredient("locomotive", "electronic-circuit", 10, 10)
  rm.AddIngredient("locomotive", "transceiver", 2, 2)
else
  rm.RemoveIngredient("locomotive", "electronic-circuit", 5, 5)
  rm.AddIngredient("locomotive", "transceiver", 1, 1)
end
...
if mods["FreightForwarding"] then
  ...
  rm.RemoveIngredient("train-stop", "electronic-circuit", 5, 5)
  rm.AddIngredient("train-stop", "transceiver", 1, 1)
end
...
if mods["space-exploration"] and mods["bzgold"] then
  ...
  rm.ReplaceIngredient("rail-signal", "iron-plate", parts.wire, 1, 1)
  rm.ReplaceIngredient("rail-chain-signal", "iron-plate", parts.wire, 1, 1)
...
rm.ReplaceProportional("electric-engine-unit", "electronic-circuit", "advanced-cable", 1, 1)
...
rm.ReplaceIngredient("substation", "copper-plate", "advanced-cable", 5, 5)
rm.ReplaceIngredient("substation", "cooling-fan", "hv-power-regulator", 1, 1)
rm.RemoveIngredient("substation", "advanced-circuit", 5, 5)
...
rm.RemoveIngredient("electric-furnace", "advanced-circuit", 5, 5)
rm.AddIngredient("electric-furnace", "hv-power-regulator", 1, 1)
rm.AddIngredient("electric-furnace", "advanced-cable", 3, 3)

rm.RemoveIngredient("flying-robot-frame", "electronic-circuit", 3, 3)
rm.AddIngredient("flying-robot-frame", "transceiver", 1, 1)

tf.removePrereq("construction-robotics", "injection-molding")
tf.removePrereq("logistics-robotics", "injection-molding")
rm.RemoveIngredient("roboport", "advanced-circuit", 20, 20)
rm.ReplaceIngredient("roboport", "cooling-fan", "hv-power-regulator", 5, 5)
...
if parts.heavyGyro then
  rm.RemoveIngredient("flying-robot-frame", "electronic-components", 9999, 9999)

  rm.ReplaceIngredient("artillery-shell", "radar", "transceiver", 1, 1)
  rm.RemoveIngredient("construction-robot", "electronic-circuit", 1, 1)
  rm.RemoveIngredient("construction-robot", "iron-gear-wheel", 1, 1)
  rm.RemoveIngredient("logistic-robot", "advanced-circuit", 1, 1)
  rm.RemoveIngredient("logistic-robot", "bearing", 1, 1)
else
  rm.AddIngredient("artillery-shell", "transceiver", 1, 1)
end
...
if rm.CheckIngredient("assembling-machine-3", "speed-module") then
  rm.AddIngredient("assembling-machine-3", "hv-power-regulator", 1, 1)
  rm.ReplaceIngredient("assembling-machine-3", "speed-module", "integrated-circuit", 20, 20)
else
  rm.AddIngredient("assembling-machine-3", "hv-power-regulator", 1, 1)
  rm.ReplaceIngredient("advanced-machining-tool", "speed-module", "integrated-circuit", 5, 5)
  rm.RemoveIngredient("advanced-machining-tool", "advanced-circuit", 99, 99)
end

tf.addPrereq("laser", "high-voltage-equipment")
rm.RemoveIngredient("laser-turret", "battery", 12, 12)
rm.ReplaceProportional("laser-turret", "electronic-circuit", "hv-power-regulator", 0.1)
rm.ReplaceIngredient("distractor-capsule", "advanced-circuit", "advanced-cable", 3, 3)
rm.ReplaceIngredient("destroyer-capsule", "speed-module", "hv-power-regulator", 1, 1)

tf.addPrereq("nuclear-power", "high-voltage-equipment")
rm.AddIngredient("steam-turbine", "hv-power-regulator", 1, 1)
...
rm.AddIngredient("solar-panel-equipment", parts.wire, 6, 6)
rm.AddIngredient("battery-equipment", parts.wire, 6, 6)

tf.addPrereq("battery-mk2-equipment", "high-voltage-equipment")
rm.AddIngredient("battery-mk2-equipment", "hv-power-regulator", 5, 5)

rm.AddIngredient("fusion-reactor-equipment", "advanced-cable", 100)
rm.ReplaceIngredient("fusion-reactor-equipment", "processing-unit", "hv-power-regulator", 50, 50)

rm.AddIngredient("electric-foundry", "hv-power-regulator", 1, 1)
rm.AddIngredient("electric-foundry", "advanced-cable", 6, 6)

rm.AddIngredient("rocket-silo", "hv-power-regulator", mods["LunarLandings"] and 10 or 50, mods["LunarLandings"] and 10 or 50)
if not mods["Krastorio2"] then
  rm.ReplaceIngredient("rocket-fuel", "light-oil", "nitric-acid", 10, 10)
end

rm.ReplaceIngredient("rocket-control-unit", "speed-module", "transceiver", 1, 1)
if mods["space-exploration"] then
  rm.AddIngredient("satellite", "transceiver", 25, 25)
else
  if mods["LunarLandings"] then
    rm.AddIngredient("rocket-control-unit", "advanced-circuit", 4, 4)
    tf.addPrereq("rocket-control-unit", "chemical-science-pack")
  else
    rm.AddIngredient("satellite", "transceiver", 100, 100)
    rm.AddIngredient("rocket-control-unit", "integrated-circuit", 10, 10)
  end
end

if mods["vtk-deepcore-mining"] then
  rm.ReplaceIngredient("vtk-deepcore-mining-gold-ore-chunk-refining", "sulfuric-acid", "nitric-acid", 20, 20)
end
...
if mods["LunarLandings"] then
  ...
  rm.AddIngredient("ll-arc-furnace", "hv-power-regulator", 5, 5)
  rm.AddIngredient("ll-arc-furnace", "advanced-cable", 25, 25)

  if data.raw.item["micron-tolerance-components"] then
    rm.AddIngredient("ll-quantum-resonator", "hv-power-regulator", 10, 10)
  else
    rm.AddIngredient("ll-quantum-resonator", "hv-power-regulator", 5, 5)
    rm.AddIngredient("ll-quantum-resonator", "advanced-cable", 25, 25)
  end

  rm.RemoveIngredient("ll-blank-data-card", "advanced-circuit", 2, 2)
